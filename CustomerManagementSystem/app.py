...
def init_db():
    ...
    try:
        conn.executescript('''
            ...
            photo_path TEXT,
            photo_type TEXT CHECK(photo_type IN ('before', 'after')) DEFAULT 'before',
            created_at DATETIME DEFAULT (datetime('now', 'localtime')),
            ...
            CREATE TABLE IF NOT EXISTS billing (
                ...
                last_editor TEXT,
                reminder_date DATETIME,  -- 新增通知日期
                reminder_user_id INTEGER,  -- 新增通知對象
                signed_reminder_user_id INTEGER,  -- 新增客戶已回簽後的通知對象
...
def upgrade_tables():
    conn = get_db()
    try:
        # 升級 billing 表：新增 reminder_date、reminder_user_id、signed_reminder_user_id 與 notified 欄位
        ...
        if "reminder_user_id" not in columns:
            conn.execute("ALTER TABLE billing ADD COLUMN reminder_user_id INTEGER")
            conn.commit()
            print("已新增 reminder_user_id 欄位到 billing 表")
        if "signed_reminder_user_id" not in columns:
            conn.execute("ALTER TABLE billing ADD COLUMN signed_reminder_user_id INTEGER")
            conn.commit()
            print("已新增 signed_reminder_user_id 欄位到 billing 表")
        ...
        if "created_by" not in columns:
            conn.execute("ALTER TABLE repairs ADD COLUMN created_by TEXT")
            conn.commit()
            print("已新增 created_by 欄位到 repairs 表")     
        # 升級 users 表：新增 avatar 與 line_id 欄位
        ("PRAGMA table_info(users)")
        cursor = conn.execute("PRAGMA table_info(repair_photos)")
        columns = [row["name"] for row in cursor.fetchall()]
        if "photo_type" not in columns:
            conn.execute("ALTER TABLE repair_photos ADD COLUMN photo_type TEXT CHECK(photo_type IN ('before', 'after')) DEFAULT 'before'")
            conn.commit()
            print("已新增 photo_type 欄位到 repair_photos 表")
...
def add_repair(customer_id):
    if request.method == 'POST':
        ...
        try:
            cur = conn.cursor()
            cur.execute('''
    INSERT INTO repairs (customer_id, description, status, price, created_by)
    VALUES (?, ?, ?, ?, ?)
''', (data['customer_id'], data['description'], data['status'], price, session.get('user', {}).get('username', 'unknown')))
...
def billing_next(id):
    conn = get_db()
    try:
        record = conn.execute("SELECT b.*, c.name as customer_name FROM billing b JOIN customers c ON b.customer_id = c.id WHERE b.id=?", (id,)).fetchone()
        if not record:
        ...
        if current_step < 5:
            ...
            conn.commit()
            # 檢查是否到達「客戶已回簽」階段（current_step=2）
            if current_step == 2 and record['signed_reminder_user_id']:
                user = conn.execute("SELECT * FROM users WHERE id=?", (record['signed_reminder_user_id'],)).fetchone()
                if user and user['line_id']:
                    message = f"請款流程提醒：{record['customer_name']} 的請款流程已到達「客戶已回簽」階段，請確認後續流程！"
                    status, resp_text = send_line_message(message, to=user['line_id'])
                    print("LINE 推送狀態給", user['username'], ":", status, resp_text)
...
def edit_billing(id):
    ...
    try:
        ...
        if request.method=='POST':
            ...
            reminder_user_id = request.form.get('reminder_user_id')
            signed_reminder_user_id = request.form.get('signed_reminder_user_id')
            ...
            conn.execute("UPDATE billing SET unpaid_amount=?, current_step=?, last_updated=?, last_editor=?, reminder_date=?, reminder_user_id=?, signed_reminder_user_id=? WHERE id=?",
                         (unpaid_amount, current_step, now_str, last_editor, reminder_date if reminder_date else None, reminder_user_id, signed_reminder_user_id, id))
            conn.commit()
        ...
        return render_template("billing_edit.html", record=record, users=users)
    except Exception as e:
...
def add_billing():
    ...
    try:
        ...
        if request.method=='POST':
            ...
            now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            cur = conn.cursor()
            cur.execute("INSERT INTO billing (customer_id, unpaid_amount, current_step, last_updated, reminder_date, reminder_user_id, signed_reminder_user_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
                        (customer_id, 0.0, 0, now_str, reminder_date if reminder_date else None, reminder_user_id, signed_reminder_user_id))
            ...
            return redirect(url_for('billing'))
        return render_template("billing_add.html", available_customers=available_customers, users=users)
    except Exception as e:
...
def inject_notifications():
    conn = get_db()
    try:
        # 取得尚未推送 LINE 通知的提醒記錄（完整比對日期時間）
        reminders = conn.execute("""
            SELECT b.*, c.name as customer_name 
            FROM billing b
            JOIN customers c ON b.customer_id = c.id
            WHERE b.reminder_date IS NOT NULL 
              AND b.reminder_date <= datetime('now','localtime')
              AND b.notified = 0
            ORDER BY b.reminder_date ASC
        """).fetchall()
        for reminder in reminders:
            if reminder['reminder_user_id']:
                user = conn.execute("SELECT * FROM users WHERE id=?", (reminder['reminder_user_id'],)).fetchone()
                if user and user['line_id']:
        ...
        conn.commit()
        # 再查詢所有提醒資料（不論已通知與否）供前端顯示，完整比對日期時間
        reminders = conn.execute("""
            SELECT b.*, c.name as customer_name 
            FROM billing b
            JOIN customers c ON b.customer_id = c.id
    ...
    if data and "events" in data:
        for event in data["events"]:
            ...
            if source_type == "group":
                ...
                if group_id:
                    print("接收到群組訊息，群組 ID:", group_id)
            else:
                # 處理個人訊息
                user_id = source.get("userId")
                if user_id:
                    print("接收到個人訊息，使用者 ID:", user_id)
    return "OK", 200
